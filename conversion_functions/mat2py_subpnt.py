...
# Since MATLAB can give outputs for more values of "et", while Python can only handle one value at a time, a "for"
# cycle is required in mat2py_subpnt when n > 1: the outputs are charged in np.ndarrays("spoint" and "srfvec) and
# nd.array(trgepc) in order to make Python's output consistent with MATLAB's.
